<template>
	<view class="">
		...
		<view class="lists" v-for="(item,index) in huizi_arr_list" :key="index">
			<view class="lists-num">
				第<text>{{ item.self_payment_num}}</text>期
			</view>
			<view class="lists-detail">
				<view class="lists-detail-y ">
					{{ item.cost == 0?'未缴':'已缴'}}
				</view>
				<view class="lists-detail-m ">
					缴费：{{ item.cost}} 元
				</view>
				<view class="lists-detail-m " v-if="huizi_arr.subitems_timemodel == '新历' ">
					时间：{{ item.year}}-{{ item.month}}-{{ item.day}}
				</view>
