...
/* harmony import */ var _index_vue_vue_type_template_id_44b8c4e5___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./index.vue?vue&type=template&id=44b8c4e5& */ 58);
      ...
      success: function success(res) {
        for (var i = 0; i < res.data.self_huzi.length; i++) {
          if (res.data.self_huzi[i].id == option.id) {
            ...
            var payment_num = Number(data_arr.payment_num); //当前已缴期数（可能存在缴费时间已过，但未缴费情况）
            ...
            if (data_arr.isfull) {
              for (var _j = 0; _j < current_num - 1; _j++) {
              ...
              console.log(_this.earned_surplus);
              _this.huizi_arr = res.data.self_huzi[i];
              console.log(_this.huizi_arr);
