...
var toDay = {
  ...
  getToMonthData: function getToMonthData(item) {
    var keysArray = uni.getStorageSync('huizi_keys');
    var len = keysArray.length;
    var date = "".concat(item.year, "-").concat(item.month);
    var dateData = {};
    var c_huizi_arr = {};
    var self_day_data = [];
    if (keysArray) {
        ...
        uni.getStorage({
          ...
          success: function success(res) {
            ...
            for (var j = 0; j < huiziLen; j++) {
              for (var index in res.data.self_huzi[j].huizi_arr) {
                var time = "".concat(
                res.data.self_huzi[j].huizi_arr[index].year, "-").concat(res.data.self_huzi[j].huizi_arr[index].month - 1);
                if (date == time) {
                  dateData = res.data.self_huzi[j].huizi_arr[index];
                  c_huizi_arr = _objectSpread({}, res.data.self_huzi[j]);
                  c_huizi_arr.huizi_arr = [];
