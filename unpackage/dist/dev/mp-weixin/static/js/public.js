						...
						for (let j = 0; j < huiziLen; j++) {
							if (res.data.self_huzi[j].subitems_timemodel == '新历') {
								...
								for (let index in res.data.self_huzi[j].huizi_arr) {
									let time =
										`${res.data.self_huzi[j].huizi_arr[index].year}-${res.data.self_huzi[j].huizi_arr[index].month -1 }-${day}`;
								...
								if (day == item.lunar) {
									for (let index in res.data.self_huzi[j].huizi_arr) {
										if (res.data.self_huzi[j].huizi_arr[index].year == item.year && res.data.self_huzi[j].huizi_arr[index].month -
											1 == item.month) {
											dateData = res.data.self_huzi[j].huizi_arr[index];
											c_huizi_arr = { ...res.data.self_huzi[j]
											};
